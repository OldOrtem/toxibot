...
async def send(message: types.Message, state: FSMContext):
    ...
    await message.answer("Йо, огрызок, хочешь поболтать?")
    ...
    await bot.send_message(chat_id=user_id, text="Здаров, родной, го поболтаем?", reply_markup=keyboard)
    await Send.next()
